...
var Lightbox = {
  ...
  submit: function($form, callback) {
    ...
    // ORIGINAL - $(this) does not work in this context:
    //$(this).find('.modal-body').html(vufindString.loading + "...");
	// Removed by AK Bibliothek Wien - End
    
	// Added by AK Bibliothek Wien - Begin
    if ($('#modal').find('.btn-primary').length) { // Add loading spinner beside submit button on form submission
    	$('#modal').find('.btn-primary').after('&nbsp;&nbsp;<i class="fa fa-spinner fa-spin"></i>');
    	$('#modal').find('.btn-primary').prop( "disabled", true ); // Disable the submit button as it was clicked already
    	
    } else { // If no primary button was found, replace whole modal-body with a loading spinner
		$('#modal').find('.modal-body').html('<div style="text-align: center;"><i class="fa fa-spinner fa-spin fa-2x"></i></div>');
	}
