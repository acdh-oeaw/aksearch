<?php
/**
 * Extension SideFacets Recommendations Module
 *
 * PHP version 5
 *
...
class SideFacets extends \VuFind\Recommend\SideFacets 
{
     ...
     * hierarchical facets
     */
    public function __construct(\VuFind\Config\PluginManager $configLoader, HierarchicalFacetHelper $facetHelper = null) {
    ...
    public function setConfig($settings) {
        ...
        // $akConfig = $this->configLoader->get('AKsearch');
        
        

        // All standard facets to display:
        $this->mainFacets = isset($config->$mainSection) ? $config->$mainSection->toArray() : [];
        ...
        if (isset($config->SpecialFacets->genericRange)) {
            $this->genericRangeFacets = $config->SpecialFacets->genericRange->toArray();
        }
        if (isset($config->SpecialFacets->numericRange)) {
            $this->numericRangeFacets = $config->SpecialFacets->numericRange->toArray();
        }
        ...
        $this->checkboxFacets = ($checkboxSection && isset($config->$checkboxSection)) ? $config->$checkboxSection->toArray() : [];
        ...
        if (isset($config->Results_Settings->showMore)) {
            $this->showMoreSettings = $config->Results_Settings->showMore->toArray();
        }
        ...
        if (isset($config->SpecialFacets->hierarchical)) {
            $this->hierarchicalFacets = $config->SpecialFacets->hierarchical->toArray();
        }

        // Hierarchical facet sort options:
        if (isset($config->SpecialFacets->hierarchicalFacetSortOptions)) {
            $this->hierarchicalFacetSortOptions = $config->SpecialFacets->hierarchicalFacetSortOptions->toArray();
        }
    }
    ...
    public function getFacetSet() {
        ...
        $akConfigFacetInfoArr = ($akConfigFacetInfoObj != null && !empty($akConfigFacetInfoObj)) ? $akConfigFacetInfoObj->toArray() : null; // Convert object to array
       	...
       	if ($akConfigFacetInfoArr != null && !empty($akConfigFacetInfoArr)) {
	        ...
	        // specific facet or not. We will use them to display the button for the tooltip opening, the tooltip heading and
	        // the tooltip text.
	        foreach ($facetSet as $facetField => &$facet) {
	        	if (key_exists($facetField, $akConfigFacetInfoArr)) {
	        		$tooltipHeading = key($akConfigFacetInfoArr[$facetField]);
	        		$tooltipText = $akConfigFacetInfoArr[$facetField][$tooltipHeading];
	        		$facet['akFacetInfoTooltipHeading'] = $tooltipHeading;
	        		$facet['akFacetInfoTooltipText'] = $tooltipText;
	        	}	
	        }
       	}
